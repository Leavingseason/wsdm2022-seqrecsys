...
## source /anaconda/bin/activate && conda activate py37_tensorflow && bash task_B.sh
...
### for quick test of the pipeline, data parameters are set to a small number. For full data, please note the comments in Line 142 of generate_dataset_B.py
python generate_dataset_B.py $raw_data_path $output_path'/'$output_tag

python generate_features_dataset_A.py $raw_data_path'/edges_train_B.csv' $output_path'/'$output_tag $output_path'/'$output_tag'_feature'

python append_cont_feature.py  $output_path'/'$output_tag $output_path'/'$output_tag'_feature' $output_path'/'$output_tag'_full'

python run_seqreco_B.py $output_path'/'$output_tag'_full'