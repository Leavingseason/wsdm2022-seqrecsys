...
class SequentialBaseModel(BaseModel):
    ...
    def _build_graph(self):
        ...
        with tf.compat.v1.variable_scope("sequential") as self.sequential_scope:
            ...
            if self.hparams.use_cont_feat:
                if self.hparams.use_useritem_type == 1:
                    print('##INFO: concat continuous features in the input of DNN ranker.')
                    model_output = tf.concat([model_output, self.iterator.cont_feat], 1)
                elif self.hparams.use_useritem_type == 0:
                    print('##INFO: only use continuous features in the input of DNN ranker.')
                    model_output = self.iterator.cont_feat
                elif self.hparams.use_useritem_type == 2:
                    print('##INFO: use dot product and continuous features in the input of DNN ranker.')
                    user_vec, item_vec = tf.split(model_output, [hparams.hidden_size, hparams.hidden_size], 1)
                    user_norm = tf.math.reduce_sum(user_vec*user_vec, axis=-1, keepdims=True) + 1e-6
                    item_norm = tf.math.reduce_sum(item_vec*item_vec, axis=-1, keepdims=True) + 1e-6
                    vec_scores = tf.math.reduce_sum(tf.math.multiply(user_vec / user_norm, item_vec / item_norm), axis=1, keepdims=True)
                    model_output = tf.concat([vec_scores, self.iterator.cont_feat], 1)
    ...
    def fit(
        ...
        # file_iterator = self.iterator.load_data_from_file_training(
        #     train_file,
        #     min_seq_length=self.min_seq_length,
        #     batch_num_ngs=0 if not self.need_sample else self.train_num_ngs,
        # )

        
        step = 0 ## hparams.epochs
        for batch_data_input in self.iterator.load_data_from_file_training(
            train_file,
            min_seq_length=self.min_seq_length,
            batch_num_ngs=0 if not self.need_sample else self.train_num_ngs,
        ):
            step += 1 
            if self.hparams.epochs < step:
                break
            ...
            if step % self.hparams.show_step == 0:
                ...
                if valid_res[eval_metric] > best_metric:
                    ...
                    if early_stop > 0 and step - self.best_epoch >= early_stop:
                        print("early stop at epoch {0}!".format(step))
                        break
                ...
                if self.hparams.save_model and self.hparams.MODEL_DIR:
                    ...
                    checkpoint_path = self.saver.save(
                            sess=train_sess,
                            save_path=os.path.join(self.hparams.MODEL_DIR, "step_" + str(step)),
                        )
                    if progress: 
    ...
    def run_eval(self, filename, num_ngs=None):
