...
class BaseModel:
    ...
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        if self.hparams.init_item_emb:
            self._inti_item_emb_with_w2v()
    ...
    def _inti_item_emb_with_w2v(self):
        file = self.hparams.file_w2v
        item_cnt = len(self.iterator.itemdict)
        item_emb = np.zeros((item_cnt, self.item_embedding_dim), dtype=np.float32)
        valid_cnt = 0
        with open(file, 'r') as rd:
            while True:
                ...
                if not line:
                    break
                words = line[:-1].split(' ')
                idx = self.iterator.itemdict[words[0]] if words[0] in self.iterator.itemdict else -1
                if idx >= 0:
                    valid_cnt += 1
                    vec = [float(a) for a in words[1:]]
                    item_emb[idx] = np.array(vec, dtype=np.float32)
        print('Valid Cnt in init with w2v : {0}'.format(valid_cnt))
        self.item_lookup.load(item_emb, self.sess)
    ...
    def _compute_data_loss(self):
        print('##INFO: self.hparams.loss is {0}'.format(self.hparams.loss))
